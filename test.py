...
menu = ["Home", "1.Phân Tích Đánh Giá và Số Lượng Đánh Giá", "2.So Sánh Đánh Giá Giữa Khách Sạn 4 Sao và 5 Sao", "3.Phân Tích Nội Dung và Phản Hồi Từ Chủ Khách Sạn",
        "4.Tương Tác của Người Đánh Giá", "5.So Sánh Các Khía Cạnh Khu Vực Địa Lý", "6.Xu Hướng và Thay Đổi Theo Thời Gian", "7.Phân Tích Mối Quan Hệ Giữa Các Biến"]
...
def review_count_by_city_heatmap(df_4star, df_5star):
    ...
    sns.heatmap(city_counts, annot=True, cmap="YlGnBu", fmt=".0f", cbar=True, linewidths=.5, linecolor='black')
    plt.xlabel('Loại khách sạn')
    plt.ylabel('Thành phố')
    plt.title('Số lượng đánh giá theo thành phố cho khách sạn 4 sao và 5 sao')
    st.pyplot(plt)
...
if choice == "Home":
    ...
    if st.button("Generate Chart"):
        if user_input:
            ...
            The data is stored in a DataFrame named "data" so do not create sample data, just assume data is there.
            Include a variable "comment" in the code with a short explanation or insights from the chart. The comment should be in Vietnamese.
            '''
            try:
...
elif choice == "7.Phân Tích Mối Quan Hệ Giữa Các Biến":
